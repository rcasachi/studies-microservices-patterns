...
export default class ProcessPayment {
  ...
  constructor(readonly registry: Registry) {
    ...
    this.paymentRepository = registry.inject('paymentRepository')
    this.queue = registry.inject('queue')
  }

  async execute(input: Input): Promise<void> {
    const output = await this.paymentRepository.createTransaction({
      email: input.email, creditCardToken: input.creditCardToken, price: input.price,
    })

    const transaction = Transaction.create(
      input.ticketId, input.eventId, output.tid, input.price, output.status
    )
